...
(function () {
  ...
  const CONFIG = {
    ...
    buttonId: 'btnAbLoop',
    // ---- custom config start ----
    // spacingMode / symmetricExtraGapEm / BALANCED_FIXED_OFFSET removed --
    // superseded by the plugin's General/Individual Centered Gap system,
    // see the PLUGIN ADAPTER section above and applySpacing() below.
    // centeredGapEm: baseline default is 0 (no extra gap) for standalone
    // usage, same "opt-in, not opt-out" default the plugin itself uses.
    centeredGapEm: 0,
  ...
  // Simplified: baseline is 0 (no gap), General/Individual Centered Gap
  // (see PLUGIN ADAPTER above) applies symmetrically on both sides.
  // Replaces the old spacingMode/symmetricExtraGapEm/BALANCED_FIXED_OFFSET
  // logic entirely.
